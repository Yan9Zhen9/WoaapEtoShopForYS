...
class YsClient
{
    const YS_ADD_ORDER_URL = "data-api/v1/order/add_order";
    const EVENT_NODE_LIST = [
        'order_create_success' => 'create_success',
        'order_pay_success' => 'pay_success',
        'order_deliver' => 'delivery_success',
        'order_success' => 'receive_success',
    ...
    {
        $this->appId = config('orderforys')['app_id'] ?? throw new \Exception('app_id 未配置');
        $this->appSecret = config('orderforys')['app_secret'] ?? throw new \Exception('app_secret 未配置');
    }

    public function handle($dataSourceId, $order_info, $node)
    {
        try {
            $url_params = SignService::getUrlSing($this->appId, $this->appSecret);
            $url = config('orderforys')['ys_domain'] . self::YS_ADD_ORDER_URL . '?' . $url_params;
            $params['dataSourceId'] = $dataSourceId;
            $params['orders'][] = self::getOrders($order_info, $node);
            $result = YouShuHttpUtil::post($url, $params);
            if (isset($result['retcode']) && $result['retcode'] === 0) {
                return $result;
            }
        } catch (\Exception $e) {
            if ($e instanceof ConnectException || $e instanceof ServerException) {
                $result['ys_client_errmsg'] = $e->getMessage();
                return $result;
            }
        }
    }
     ...
     * @return array
     */
    public static function getOrders($data, $order_status)
    {
        $result = [];
        switch (self::EVENT_NODE_LIST[$order_status]) {
     ...
     * @return array
     */
    public static function createUnPaySuccessOrder($data)
    {
     ...
     * @return array
     */
    public static function createPaySuccessOrder($data)
    {
        $list = self::_makeCommonDataPay($data);//获取公共参数
     ...
     * @return array
     */
    public static function createDeliverySuccessOrder($data)
    {
     ...
     * @return array
     */
    public static function createReceiveSuccessOrder($data)
    {
        $list = self::_makeCommonDataDeliver($data);//获取公共参数
     ...
     * @return array
     */
    public static function createRefundSuccessOrder($data)
    {
        $goodsInfo = [];
        $goods_amount_total = 0.00;
